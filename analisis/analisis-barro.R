...
# Creación de la tabla "r_personajesCompartidos.csv"
##########
...
# Dado un personaje, cual es el personaje que más se parece a él? Esto se hace con within-graph similarity. Neumann kernel (también llamado walk proximity)
#Modificamos la estructura del grafo haciendo tottering para reducir el sesgo de la similaridad debido a los ciclos cortos.
#NO SE HACE totterizar porque la MATRIZ TOTTERIZADA es DEMASIADO GRANDE para ejecutar el código

# TotterTransform <- function(gf) {
#   df <- as_data_frame(gf)
#   names(df) <- c("edge-from", "edge-to")
#   tottering <- df %>% mutate(`edge-to` = paste(`edge-from`, `edge-to`, sep = "-"))
#   for(inp in unique(df$`edge-from`)) {
#     mids <- df %>% filter(`edge-from` == inp) %>% .$`edge-to`
#     for(mid in mids) {
#       out <- df %>% filter(`edge-from` == mid, `edge-to` != inp)
#       if(dim(out)[1] > 0) {
#         tottering <- bind_rows(tottering, out %>% mutate(`edge-to` = paste(`edge-from`, `edge-to`, sep = "-")) %>% mutate(`edge-from` = paste(inp, mid, sep = "-")))
#       }
#     }
#   }
#   tottering <- bind_rows(tottering, tibble(`edge-from` = V(gf)$name, `edge-to` = NA))
#   gfTott <- graph_from_data_frame(tottering)
#   gfTott <- delete.vertices(gfTott, "NA")
#   return(gfTott)
# }

# gf_tott <- TotterTransform(gf)

A <- as.matrix(as_adjacency_matrix(gf))

MIdg <- max(degree(gf,mode="in"))
MOdg <- max(degree(gf,mode="out"))
max_gamma <- 1/(min(MIdg,MOdg)+1)

K_ <- round(solve(diag(rep(1,dim(A)[1]))-max_gamma*t(A)%*%A),3) # Common incoming paths similarity
T_ <- round(solve(diag(rep(1,dim(A)[1]))-max_gamma*A%*%t(A)),3) # Common outgoing paths similarity

p1 <- "Jörmundgander"
p1 <- str_replace_all(p1, "[^[:alpha:]]", ".")
sort(K_[p1,which(row.names(K_) != p1)], decreasing = T)[1:5]
sort(T_[p1,which(row.names(T_) != p1)], decreasing = T)[1:5]
# Funciona regulero debido a que no se ha podido realizar la totterización


# degree como medida de centralidad
# NO ES INTERESANTE
# personajes_sub$degree <- degree(gf_sub, mode = "in")
# personajes_sub %>%
#   top_n(15, degree) %>%
#   ggplot(aes(x = degree, y = reorder(Nombre, degree))) +
#   geom_col() +
#   labs(y = "Personaje") +
#   theme_light() +
#   theme(panel.grid.major.y = element_blank())
...
# puntuaciones HITS (hub y authority) [NO LO VEO INTERESANTE]
# V(gf)$hs <- hub_score(gf)$vector
# personajes$hs <- V(gf)$hs
# personajes %>%
#   top_n(15, hs) %>%
#   ggplot(aes(x = hs, y = reorder(Nombre, hs))) +
#   geom_col() +
#   labs(y = "Personaje") +
#   theme_light() +
#   theme(panel.grid.major.y = element_blank())
# V(gf)$as <- authority_score(gf)$vector
# personajes$as <- V(gf)$as
# personajes %>%
#   top_n(15, as) %>%
#   ggplot(aes(x = as, y = reorder(Nombre, as))) +
#   geom_col() +
#   labs(y = "Personaje") +
#   theme_light() +
#   theme(panel.grid.major.y = element_blank())
...
# Dado el CORE de una mitología, ver cual es otro CORE más similar a ella (NO SE PUEDE HACER, DIRECT PRODUCT DE MATRICES DEMASIADO GRANDE)


# Clustering
# NO ENCUENTRO LOS PAQUETES
# library("GraphClusterAnalysis")
# library(RBGL) 

# Shared Nearest Neighbor
# clusterSNN <- SNN_Clustering(gf_f_sub, tau = 3)
# # Mayor cluster
# as.tibble(clusterSNN$Clusters) %>%
#   count(ClusterID) %>%
#   arrange(-n) %>% .$ClusterID %>% .[1] -> c_id
# as.tibble(clusterSNN$Clusters) %>%
#   filter(ClusterID == c_id) %>% .$VertexID -> aux_pjs
# V(gf_f_sub)$name[aux_pjs + 1]
# # Personajes con más vecinos comunes
# aux_pjs_sub <- tibble(nombre = V(gf_f_sub)$name, id = 0:(length(V(gf_f_sub)) - 1))
# as.tibble(clusterSNN$`Edges from SNN that satisfy tau`) %>%
#   arrange(-`Number of Shared Neighbors`) %>%
#   head(5) %>%
#   left_join(aux_pjs_sub, by = c("Vertex" = "id")) %>%
#   left_join(aux_pjs_sub, by = c("V2" = "id")) %>%
#   select(-Vertex, -V2)

# Betweenness Centrality Clustering (con la estructura scale-free del grafo, solo se consiguen aislar algunos grupos de personajes)
# Poco interesante, dificil de escalar
# clusterBC <- betweennessBasedClustering(gf_f_sub, mode = "edge", threshold = 0.05)

# Highly Connected Subgraph Clustering (se obtienen demasiados "singletons")

# Maximal Clique Enumeration
# clusterMC <- maximalCliqueEnumerator(gf_f)
# # Cliques (mínimo 3 nodos) que contienen el personaje
# p <- "Pan"
# p <- str_replace_all(p, "[^[:alpha:]]", ".")
# for(i in clusterMC$maxCliques) {
#   if(p %in% i & length(i) > 2) {
#     print(i)
#   }
# }
